...
#[test]
fn test_ap() {
    assert!(0.525_f32 - ap(vec![7, 17, 9, 42, 5], vec![5, 7, 12, 42]).abs() < 0.1_f32);
}
#[test]
fn test_p_at_k() {
    assert!(
    ...
    assert!(0_f32 - p_at_k(vec![5, 3, 6, 1, 2], vec![1, 2, 5, 6, 7, 8], 0).abs() < 0.1_f32);
    assert!(0.75 - p_at_k(vec![5, 3, 6, 1, 2], vec![1, 2, 5, 6, 7, 8], 4).abs() < 0.1_f32);
    assert!(0.5 - p_at_k(vec![5, 3, 6, 1, 2], vec![1, 2, 5, 6, 7, 8], 8).abs() < 0.1_f32);
}
